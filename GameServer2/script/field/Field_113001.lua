...
function Field_113001:OnSensorEnter_10(Actor) -- 만넬로스가 제물의 영혼을 흡수
	...
	if (Actor:IsPlayer() == true) then		
		...
		Lich:MoveToMarker(5)
		Lich:NonDelaySay("$$Field_113001_21",2.7) -- 내 마력의 양분이 되어라
...
function Field_113001:OnDie(DespawnInfo) 
	...
	if (DespawnInfo.NPCID == 113244) then -- 제물 모두 죽으면 이벤트 시작
		if (this:GetNPCCount(113244) == 0) then
			...
			Lich:Say("$$Field_113001_44",6.5) -- (웃음) 이로써 내 육체가 모두 복구되었다.
			Lich:Wait(3)
			Lich:Say("$$Field_113001_46",3.1)	-- 곧 모든 마력을 되찾을 수 있겠군
	...
	if (DespawnInfo.NPCID == 113079) then -- [1] 게리온이 죽으면 : 이벤트 시작
		...
		Lich:Say("$$Field_113001_53",3.1) -- 이놈도 결국 쥐새끼에 불과했군
		Lich:Say("$$Field_113001_54",4.5)	-- 할 수 없지. 내가 직접 너희의 영혼을 흡수해 주마
		Lich:Say("$$Field_113001_55",2.7) -- 내 마력의 양분이 되어라
		Lich:UseTalentSelf(251200205)
...
function Field_113001:OnSensorEnter_11(Actor) -- 센서에 닿으면 이벤트 1
	...
	if (Actor:IsPlayer() == true) then	
		Iesent:Say("$$Field_113001_74",2) -- 너의 의도대로 될 거 같으냐
		Iesent:WaitFor(Lich)
		Lich:FaceTo(Iesent)
		Lich:Wait(1)
		Lich:Say("$$Field_113001_78",3.2) -- 넌 아이젠트 트리어!
		Lich:Say("$$Field_113001_79",4.7)	-- 네놈도 부활한 것인가! 하지만 고작 영혼 밖에 남지 않았군.
		Lich:Say("$$Field_113001_80",3.4) -- 육신도 없는 놈이 무엇을 어쩌겠단 것이냐!
		Lich:NextAction()
		Lich:WaitFor(Iesent)
		Iesent:Say("$$Field_113001_83",4.4) -- 나의 육신은 바스러졌지만, 내 뜻은 여기 이 자가 이어갈 것이다.
		Iesent:NextAction()
		Lich:Say("$$Field_113001_85",2.6) -- 이자가 널 불러낸 놈인가
		Lich:Say("$$Field_113001_86",3.8) -- 이런 애송이가 너의 마지막 희망이라니. 한심하군
		Lich:FaceTo(Gerion)
		Lich:Say("$$Field_113001_88",6.9) -- 거기 있는 쥐새끼. 내 부활을 도와준데 대한 보답으로, 널 멋진 전사로 거듭나게 해주마
...
function Field_113001_Fusion(Self)	-- 융합
	...
	Lich:Say("$$Field_113001_146",7.1) 	-- 이 영혼만 남은 쥐새끼가 무슨 짓을... 	
	Lich:Narration("$$Field_113001_147",3.4)	-- 게리온의 육체에서 네놈의 육체로 옮겨왔다.
	Lich:Wait(5)
	Lich:Narration("$$Field_113001_149",5.3) -- 네가 내 영혼을 흡수하기만을 얼마나 기다렸는지 넌 모를 것이다.
	Lich:Wait(6)		
	Lich:Narration("$$Field_113001_151",4.1) -- 이제 그만 반항하고, 네 육체를 나에게 바쳐라.
	Lich:Wait(5)		
	Lich:NonDelaySay("$$Field_113001_153",5.1) -- 이이이 크아아아~
...
function Field_113001:OnSensorEnter_14(Actor)
	if (Actor:IsPlayer() == true) then	
		...
		GK:Say("$$Field_113001_170",3) -- 제일스의 영혼이 만넬로스의 육체를 차지해버렸군...
		GK:Say("$$Field_113001_171",4) -- 놈이 모든 힘을 회복하게 되면, 이 세계는 큰 혼란에 빠지게 될 거야.
		GK:Say("$$Field_113001_172",3.9) -- 그 전에 만넬로스가 가지고 있는 생명의 단지를 파괴해야 해.
		GK:Say("$$Field_113001_173",2.3) -- 부탁한다... 젊은 영웅들이여.
