...
function Field_102:OnCreate()
	this:SetTimer(1, 7200, false)	-- 트롤호위
end

function Field_102:OnSensorEnter_2000000(actor)
	this:KillTimer(1);
	this:SetTimer(1, 2, false)	-- 테스트용 센서에 닫으면 트롤호위 시작
end

function Field_102:OnSensorEnter_2000001(actor)
	this:KillTimer(2);
	this:SetTimer(2, 2, false)	-- 테스트용 센서에 닫으면 오우거호위 시작
end
...
function Field_102:OnTimer(TimerID) 
	if (TimerID == 1) then	
		...
		this:SetTimer(2, 7200, false)	-- 오우거호위 시작
		
	elseif (TimerID == 2) then
		local QuestPVP = this:GetQuestPVP();
		QuestPVP:BeginEvent(2);
		this:SetTimer(1, 7200, false)	-- 트롤호위 시작			
	end
end
...
function Field_102:OnSpawn(SpawnInfo)		
	if (SpawnInfo.NPCID == 102204) then -- 순례자
		...
		SpawnInfo.NPC:ScriptSelf("Field_102_DespawnSelf")	-- 디스폰 되기	


	elseif (SpawnInfo.NPCID == 102150) then -- 트롤 제사장 로킬라 스폰
