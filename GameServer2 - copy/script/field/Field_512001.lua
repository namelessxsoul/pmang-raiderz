...
function Field_512001:OnSensorEnter_10(Actor) -- 센서에 닿으면 이벤트 1
	...
	if (Actor:IsPlayer() == true) then	
		...
		Iesent:Say("$$Field_512001_75",1.9) -- 이것이 바로 생명의 단지야
		Iesent:Say("$$Field_512001_76",3.2) -- 이 단지를 파괴하면 만넬로스는 영원히 사라지게 된다.
		Iesent:NextAction()
		Heiden:Say("$$Field_512001_78",2.3) -- 저 생명의 단지를 파괴하시게
	end
end
...
function Field_512001:OnSensorEnter_11(Actor) -- 센서에 닿으면 이벤트 2
	...
	if (Actor:IsPlayer() == true) then	
		...
		Jails:Say("$$Field_512001_98",2.9) -- 이 치욕은 반드시 값아주고 말겠다.
		Jails:NextAction()
		Jails:WaitFor(Heiden)
		Heiden:Say("$$Field_512001_101",2.6) -- 난 아렐의 성격을 잘 알지
		Heiden:Say("$$Field_512001_102",4.5) -- 아렐이 실패를 거듭한 그대에게 더 이상 기회를 줄 거 같나?
		Heiden:NextAction()
		this:PlayBGM("bgm_Scenewar_1") 
		Heiden:WaitFor(Jails)		
		Jails:Say("$$Field_512001_106",2.9) -- 아니, 우리와 아렐님의 존재를 알고 있다니.
		Jails:Say("$$Field_512001_107",2.3) -- 네놈들 정체가 뭐냐
		Jails:NextAction()
		Heiden:Say("$$Field_512001_109")		
		Heiden:NonDelaySay("$$Field_512001_110",2.8) -- 자네와는 더 이상 볼일이 없다네		
		Heiden:UseTalent(140017, Jails)
		Heiden:ScriptSelf("Field_512001_Event3",1.9) -- 잘 가게. 제일스
	end
end
...
function Field_512001_Event3(Self)	-- 생명의 단지 파괴 후에
	...
	Jails:Say("$$Field_512001_122",3.4)	-- 으아아아~ 크아아악~
